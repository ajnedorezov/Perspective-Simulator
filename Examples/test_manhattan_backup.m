...
%         plot(xx(ind), yy(ind), [colors{d} '.']);
    ...
    for xind = 1:length(xstartind)-1
        for yind = 1:length(ystartind)-1
            ...
            labels = MW.edge_labels(xindex, yindex);
            
            % Determine the angle for each dimension
            MWangles = zeros(1,3);
            for d = 1:3
                allAngles = wrapToPi(MW.angles(xindex,yindex,d));
                
                MWangles(d) = mode(allAngles(labels == d)); 
            end
            MWangles(isnan(MWangles)) = 0;
            fprintf('Current region angle set: %s\n', mat2str(MWangles*180/pi, 4))
...
%             quiver3(xpos,ypos,10,quivlength,0,0, 'r--', 'linewidth', 3)
%             quiver3(xpos,ypos,10,0,quivlength,0, 'g--', 'linewidth', 3)
%             quiver3(xpos,ypos,10,0,0,quivlength, 'b--', 'linewidth', 3)
...
% %             vpx = [(ypos - imsize(1))*abc(1,3) + focallength*abc(1,2) (xpos - imsize(2))*abc(1,3) + focallength*abc(1,1)] - [xpos ypos];
% %             vpy = [(ypos - imsize(1))*abc(2,3) + focallength*abc(2,2) (xpos - imsize(2))*abc(2,3) + focallength*abc(2,1)] - [xpos ypos];
% %             vpz = [(ypos - imsize(1))*abc(3,3) + focallength*abc(3,2) (xpos - imsize(2))*abc(3,3) + focallength*abc(3,1)] - [xpos ypos];
%             vpx = [focallength*abc(1,1) focallength*abc(1,2)] + imsize/2 - [xpos ypos];
%             vpy = [focallength*abc(2,1) focallength*abc(2,2)] + imsize/2 - [xpos ypos];
%             vpz = [focallength*abc(3,1) focallength*abc(3,2)] + imsize/2 - [xpos ypos];
%             vpx = vpx/norm(vpx);
%             vpy = vpy/norm(vpy);
%             vpz = vpz/norm(vpz);
%             quiver(xpos, ypos, quivlength*vpx(1), quivlength*vpx(2), 'r', 'linewidth', 3)
%             quiver(xpos, ypos, quivlength*vpy(1), quivlength*vpy(2), 'g', 'linewidth', 3)
%             quiver(xpos, ypos, quivlength*vpz(1), quivlength*vpz(2), 'b', 'linewidth', 3)

%             R = computeR(MWangles(1), MWangles(2), MWangles(3)) * eye(3);
...
function MW = computeManhattanWorldModel(P_edge, angGradient)
    ...
    % Twist angle
    gamma = 0;%linspace(-4, 4, 5)*pi/180;
    
    % Initialize the data sturctures to hold the results
    MW.model_likelihood = -inf(imsize(1), imsize(2));
    MW.pixel_likelihood = zeros([imsize 5]);
    MW.angles = zeros([imsize 3]);
    ...
    for a = alpha
        ...
        for b = beta
            ...
            for c = gamma
                ...
                for u = 1:imsize(1)
                    for v = 1:imsize(2)
                        % Compute the pixel_likelihood for the current
                        % pixel
                        new_pixel_likelihood = zeros(1,5);
                        new_pixel_likelihood(1) = P_edge.on(u,v) * P_ang_alpha(u,v);
                        new_pixel_likelihood(2) = P_edge.on(u,v) * P_ang_beta(u,v);
                        new_pixel_likelihood(3) = P_edge.on(u,v) * P_ang_gamma(u,v);
                        new_pixel_likelihood(4) = P_edge.on(u,v) * uniformP_ang;
                        new_pixel_likelihood(5) = P_edge.off(u,v) * uniformP_ang;
                        
                        % Compute the pixel's model likelihood
                        new_likelihood = sum(new_pixel_likelihood.*P_m);
                        
                        % See if the new MW angles are a better fit
                        if new_likelihood > MW.model_likelihood(u,v)
                            MW.model_likelihood(u,v) = new_likelihood;
                            MW.pixel_likelihood(u,v,:) = new_pixel_likelihood(:);
                            currAngleSet = [a b c];
                            MW.angles(u,v,:) = currAngleSet(:);
                            anyupdated = true;
                ...
                if anyupdated
                    fprintf('\t%d pixel likelihoods updated!\n', pixelcount);
                else
                    fprintf('\t----------> NO UPDATES!\n')
                end
            end
