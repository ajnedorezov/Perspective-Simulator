...
switch(caseNum)
    ...
    case 2
        ...
        vTimes = [1 20 60 90];
        sceneData = imread('External\CamSeq01\0016E5_07959.png');
        vid = VideoReader('Thesis Images\Chapter 5\Section 5.3.2\CamSeqMainUsingGT-DualPlot.avi');
    ...
    case 4
        ...
        svid = VideoReader('External\seq05VD\0005VD.avi');
        sceneData = read(svid,1);
        vid = VideoReader('Thesis Images\Chapter 5\Section 5.3.2\seq05VDUsingGT-DualPlot.avi');
...
threshold = 5;
fprintf('lessThan: %d\n', sum((solutionPath(valids)-directPath(valids))<-threshold))
fprintf('equalTo: %d\n', sum(abs(solutionPath(valids)-directPath(valids))<threshold))
...
msubplot = @(m,n,p,extra) subtightplot (m, n, p, [0.01 0.01], [0.0001 0.001], [0.01 0.01]);
...
%     imshow(frame(:,80:410,:) , 'Parent', ax);
    imshow(frame(31:end,170:370,:) , 'Parent', ax);
